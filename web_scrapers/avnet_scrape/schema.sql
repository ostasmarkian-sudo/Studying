...
-- Migration for databases created before quantity/price existed.
...
CREATE INDEX IF NOT EXISTS idx_products_manufacturer_part_number ON products (manufacturer_part_number);
...
ALTER TABLE products_history ADD COLUMN IF NOT EXISTS quantity INTEGER;
ALTER TABLE products_history ADD COLUMN IF NOT EXISTS price NUMERIC;
...
BEGIN
    IF OLD.data IS DISTINCT FROM NEW.data THEN
        INSERT INTO products_history (
            item_number, manufacturer_part_number, manufacturer_name, category, quantity, price, data, valid_from, valid_to
        )
        VALUES (
            OLD.item_number, OLD.manufacturer_part_number, OLD.manufacturer_name, OLD.category, OLD.quantity, OLD.price, OLD.data, OLD.updated_at, now()
        );
    END IF;
    RETURN NEW;
END;
